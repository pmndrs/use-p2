...
const tileHighNodes = ['Cube1600', 'Cube1600_1'] as const
...
function Ground() {
  ...
  return <RoundedBox radius={0.125} args={[30, 1, 10]} position-y={-0.5} material={materials.Green} />
}

const Scene = () => {
  const [{ lives }] = usePlayer()

  const [logo] = useLoader(THREE.TextureLoader, ['./kaykit_miniGame/kaykit_logo.png'])
  ...
  return (
    <>
      ...
      <div style={{ position: 'absolute', top: '50px', left: '50vw', transform: 'translate(-50%, 0)' }}>
        <div style={{ display: 'flex' }}>
          {new Array(lives < 0 ? 0 : lives).fill(0).map((l, i) => (
            <img src={'./kenney_gameicons/heart.png'} alt="" key={i} />
          ))}
        </div>
      </div>
    </>
